...
static inline void build_topics(void) {
  snprintf(TOPIC_HEATER, sizeof TOPIC_HEATER, "%s/%s/heater/state",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_HEATER_SET, sizeof TOPIC_HEATER_SET, "%s/%s/heater/set",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_STEAM, sizeof TOPIC_STEAM, "%s/%s/steam/state", GAG_TOPIC_ROOT,
           GAGGIA_ID);
  snprintf(TOPIC_CURTEMP, sizeof TOPIC_CURTEMP, "%s/%s/current_temp/state",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_SETTEMP, sizeof TOPIC_SETTEMP, "%s/%s/set_temp/state",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_PRESSURE, sizeof TOPIC_PRESSURE, "%s/%s/pressure/state",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_SHOTVOL, sizeof TOPIC_SHOTVOL, "%s/%s/shot_volume/state",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_SHOT, sizeof TOPIC_SHOT, "%s/%s/shot/state", GAG_TOPIC_ROOT,
           GAGGIA_ID);
  snprintf(TOPIC_ESPNOW_CHAN, sizeof TOPIC_ESPNOW_CHAN, "%s/%s/espnow/channel",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_ESPNOW_MAC, sizeof TOPIC_ESPNOW_MAC, "%s/%s/espnow/mac",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_ESPNOW_CMD, sizeof TOPIC_ESPNOW_CMD, "%s/%s/espnow/cmd",
           GAG_TOPIC_ROOT, GAGGIA_ID);
  snprintf(TOPIC_ESPNOW_LAST, sizeof TOPIC_ESPNOW_LAST, "%s/%s/espnow/last",
           GAG_TOPIC_ROOT, GAGGIA_ID);
}
...
static void espnow_log_cb(TimerHandle_t xTimer);
static volatile bool s_espnow_timeout_req = false;
static volatile bool s_espnow_ping_req = false;
static volatile bool s_espnow_log_req = false;
static void try_start_espnow(void);
static volatile bool s_espnow_start_req =
    false; // request to start espnow (deferred)
...
static int s_espnow_pkt_count = 0;
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  case MQTT_EVENT_DATA: {
    ...
    if (strcmp(t_copy, TOPIC_CURTEMP) == 0)
      ...
      unsigned int b[6];
      if (sscanf(d_copy, "%02x:%02x:%02x:%02x:%02x:%02x", &b[0], &b[1], &b[2],
                 &b[3], &b[4], &b[5]) == 6) {
        for (int i = 0; i < 6; ++i)
...
static void espnow_recv_cb(const esp_now_recv_info_t *info, const uint8_t *data,
  ...
  if (data_len == 1 && data[0] == ESPNOW_HANDSHAKE_ACK) {
    ...
    if (!s_espnow_log_timer)
      s_espnow_log_timer = xTimerCreate("espnow_log", pdMS_TO_TICKS(10000),
                                        pdTRUE, NULL, espnow_log_cb);
    if (s_espnow_log_timer) {
      s_espnow_pkt_count = 0;
      if (xTimerIsTimerActive(s_espnow_log_timer))
        xTimerReset(s_espnow_log_timer, 0);
      else
        xTimerStart(s_espnow_log_timer, 0);
    }
    return;
  }
...
static void try_start_espnow(void) {
  if (s_espnow_active || !s_have_espnow_mac || !s_have_espnow_chan) {
    ESP_LOGI("ESP-NOW", "Start check: active=%d have_mac=%d have_chan=%d",
             (int)s_espnow_active, (int)s_have_espnow_mac,
             (int)s_have_espnow_chan);
    return;
  }
  ...
  if (s_espnow_timer) {
    ESP_LOGI("ESP-NOW", "Starting timeout timer: %d ms",
             (int)ESPNOW_TIMEOUT_MS);
  ...
  if (!s_espnow_ping_timer) {
    s_espnow_ping_timer =
        xTimerCreate("espnow_ping", pdMS_TO_TICKS(ESPNOW_PING_PERIOD_MS),
                     pdTRUE, NULL, espnow_ping_cb);
  }
  if (s_espnow_ping_timer) {
    ESP_LOGI("ESP-NOW", "Starting ping timer: %d ms",
             (int)ESPNOW_PING_PERIOD_MS);
...
static void Wireless_Poll(void) {
  ...
  if (s_espnow_log_req) {
    s_espnow_log_req = false;
    ESP_LOGI("ESP-NOW", "%d packets received in last 10s", s_espnow_pkt_count);
    s_espnow_pkt_count = 0;
  }
  ...
  if (s_espnow_start_req) {
    ...
    if (!s_espnow_active && s_have_espnow_mac && s_have_espnow_chan) {
      ...
      if (s_mqtt && s_mqtt_connected && !s_mqtt_stopping) {
        ...
        // Mark stopping to prevent repeated stop calls; wait for DISCONNECTED
        // event
      ...
      // Step 2: disconnect WiFi STA if still connected (we use s_wifi_got_ip as
      // proxy)
      if (s_wifi_got_ip) {
        esp_wifi_disconnect();
        s_wifi_got_ip = false;
